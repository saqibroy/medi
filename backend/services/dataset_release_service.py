...
from ..models import Annotation, AnnotationHistory, DatasetRelease, DatasetReleaseEvent, Label, Project, Scan, SegmentationMask, User
...
def _release_response(release: DatasetRelease, events: list[DatasetReleaseEvent], include_manifest: bool) -> dict[str, Any]:
...
def create_release(db: Session, project_id: UUID, current_user: User) -> dict[str, Any]:
    ...
    return _release_response(release, events, include_manifest=True)
...
def list_releases(db: Session, project_id: UUID, current_user: User) -> list[dict[str, Any]]:
    ...
    return [_release_response(release, events[release.id], include_manifest=False) for release in releases]
